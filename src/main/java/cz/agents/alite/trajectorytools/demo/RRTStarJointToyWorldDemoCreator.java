...
import cz.agents.alite.trajectorytools.graph.jointtoyworld.rrtstar.ToyWorldDomain;
...
public class RRTStarJointToyWorldDemoCreator implements Creator {
    ...
    Box4dRegion bounds = new Box4dRegion(new TimePoint(0, 0, 0, 0), new TimePoint(1000, 1000, 1, 1000));
    ...
    private Trajectory[] getNominalTrajectories() {
    	for (int i=0; i < starts.length; i++) {
            Domain<TimePoint, SpatioTemporalManeuver> domain = new ToyWorldDomain(bounds, starts[i], obstacles, targets[i], targetReachedTolerance, MINSPEED, OPTSPEED, MAXSPEED, MAXPITCH, new Random(1));
    ...
    public void create() {
		...
		Mission[] missions = AgentMissionGenerator.generateSuperconflict(3,
				new SpatialPoint(500, 500, 0.5), 
				400);
