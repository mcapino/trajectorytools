...
public class ToyWorldDomain implements Domain<JointState, JointManeuver> {
    ...
    public ToyWorldDomain(Box4dRegion bounds, double separation, TimePoint[] initialPoints,
            Collection<Region> obstacles, SpatialPoint[] targets, double targetReachedTolerance,
        ...
        domains = new SpatialStraightLineDomain[initialPoints.length];
        BoxRegion box = new BoxRegion(new SpatialPoint(bounds.getCorner1().x, bounds.getCorner1().y, bounds.getCorner1().z),
        							  new SpatialPoint(bounds.getCorner2().x, bounds.getCorner2().y, bounds.getCorner2().z));
        
        
        for (int i = 0; i < initialPoints.length; i++) {
            domains[i] = new SpatialStraightLineDomain(
            		box,
                    obstacles, 
                    new BoxRegion(
                    		targets[i], 
                    		new SpatialPoint(targets[i].x + targetReachedTolerance, targets[i].y + targetReachedTolerance, targets[i].z)
                    		),
            		speed);
        }
