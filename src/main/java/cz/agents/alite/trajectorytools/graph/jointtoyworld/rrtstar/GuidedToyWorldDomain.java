...
public class GuidedToyWorldDomain implements Domain<JointState, JointManeuver> {
    ...
    public GuidedToyWorldDomain(Box4dRegion bounds, double separation, TimePoint[] initialPoints,
    ...
    private SpatialPoint getNearbyPoint(SpatialPoint p, double sdev, Random random) {
    	return new SpatialPoint(p.x + random.nextGaussian()*sdev,
    			p.y + random.nextGaussian()*sdev, p.z /*p.z + random.nextGaussian()*sdev*/);
    }
