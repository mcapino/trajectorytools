...
public class JointRRTStarLayer extends AbstractLayer {
    ...
    public static VisLayer create(final RRTStarPlanner<JointState, JointManeuver> rrtstar, final ProjectionTo2d<TimePoint> projection, 
        ...
        group.addSubLayer(StyledLineLayer.create(new StyledLineElements() {
            ...
            public Iterable<StyledLine> getLines() {
                ...
                Queue<Vertex<JointState, JointManeuver>> queue = new LinkedList<Vertex<JointState, JointManeuver>>();
                queue.add(rrtstar.getRoot());

                while(!queue.isEmpty()) {
                    Vertex<JointState, JointManeuver> current = queue.poll();
                   for (Vertex<JointState, JointManeuver> child : current.getChildren()) {
                        queue.offer(child);
                        
                        JointState jointSource = current.getState();
                        JointState jointTarget = child.getState();
        ...
        group.addSubLayer(StyledPointLayer.create(new StyledPointElements() {
            ...
            public Iterable<StyledPoint> getPoints() {
                ...
                while(!queue.isEmpty()) {
                    Vertex<JointState, JointManeuver> current = queue.poll();
                    JointState jointState = current.getState();
